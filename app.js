...
const OUTSIDE_ALEXANDRA_DELIVERY_FEE = 60;
...
function isAlexandraAddress(address) {
  return /\balex(andra)?\b/i.test(String(address || ""));
}

function deliveryFeeForAddress(address) {
  if (!String(address || "").trim()) return 0;
  return isAlexandraAddress(address) ? 0 : OUTSIDE_ALEXANDRA_DELIVERY_FEE;
}
...
function updateDeliveryFeeDisplay() {
  ...
  const address = document.querySelector('[name="address"]')?.value || "";
  const fee = deliveryFeeForAddress(address);
  feeEl.textContent = money(fee);
  noteEl.textContent = !address.trim()
    ? "Alexandra Township delivery is free. Outside Alexandra Township delivery is R60."
    : fee === 0
      ? "Alexandra Township delivery selected. Delivery is free."
      : "Outside Alexandra Township delivery selected. R60 delivery fee added.";
}
...
function renderOrders() {
  ...
  table.innerHTML = state.orders.map((order) => `
    <tr>
      <td><strong>${order.id}</strong><br>${new Date(order.createdAt).toLocaleDateString("en-ZA")}<br>Delivery: ${order.deliveryDate}</td>
      ...
      <td>
        <select class="status-select" data-order-status="${order.id}">
          ${["New", "Packed", "Out for Delivery", "Delivered", "Paid", "Cancelled"].map((status) => `<option ${status === order.status ? "selected" : ""}>${status}</option>`).join("")}
        </select>
      </td>
      <td>
...
function setDefaultDate() {
  const input = document.querySelector('[name="deliveryDate"]');
  if (!input) return;
